...
- **Full app-wide cross-tenant security audit (following stale-session fix)**: User asked to "triple check" for any remaining cross-tenant leakage after the clinician-availability incident, including forms already sent/submitted. Audited every route group (timeslots, forms/public form-fill, tasks, admin-users, insurers/non-engagement/intake/gmail/branding, Stripe, super-admin) against the threat model. Found and fixed:
  - **`POST /api/users` let the caller set an arbitrary `tenantId` in the request body** (or omit it) — a tenant admin could create a brand-new login account (with a password they chose) inside a *different* tenant, or reproduce the historical blank-tenant bug. Fixed to always force `tenantId: req.tenant?.id`, ignoring anything in the body.
  - **`POST /api/email/task-reminder` had no tenant check** — any admin could pass another tenant's `taskId` and receive that task's title/description/due date via email to an address they control. Added a `task.tenantId !== req.tenant?.id` check.
  - **`DELETE /api/non-engagement-categories/:id` deleted by ID with no tenant check** — any admin could delete another tenant's category by guessing/enumerating its ID. Fixed `storage.deleteNonEngagementCategory` to require the category's `tenantId` to match.
  - **Form submissions and drafts from the real public form-fill flow (`POST /api/form-submissions`, `POST /api/form-drafts`) were being saved with a permanently null `tenantId`.** Root cause: `requireTenant` deliberately never resolves `req.tenant` on public routes (no session to derive it from), but these two routes were passing `req.tenant?.id` anyway — always `undefined`. This means essentially all real client-submitted intake responses in production have null `tenantId`, which silently breaks the "form-responses" tenant-scoped export (it returns empty/incomplete for every tenant) even though it isn't an active cross-tenant *read* leak today. Fixed both routes (and `storage.saveOrUpdateDraft`/`submitDraft`/`createFormSubmission`) to derive `tenantId` from the already-verified `client.tenantId` instead — the same reliable-parent-entity pattern used for the timeslots fix. Added a super-admin "Fix Untagged Form Submissions" tool (`POST /api/super-admin/form-submissions/backfill-tenant-ids`) to repair existing null-tenantId rows in production from their client's tenant, since prod DB access is read-only for the agent.
  - Re-confirmed as SAFE: all timeslots endpoints, `/api/clinicians`, the public form-fill/draft *read* paths (keyed on unguessable client/form UUIDs), all `/api/forms` CRUD, tasks CRUD (aside from the reminder endpoint above), admin-users/invites, insurers, gmail-connections, intake-messages, all Stripe endpoints, and all super-admin routes.
  - **`storage.updateClinician` allowed mass-assignment of `tenantId`/`userId`** — both clinician PATCH routes (`/api/clinicians/me`, `/api/clinicians/:id`) spread most of `req.body` straight into it, so a caller could reassign a clinician's tenant or swap its linked user account through an ordinary profile edit — recreating the exact user/clinician tenant split-brain that caused the original leak. Fixed by stripping `tenantId` and `userId` inside `updateClinician` itself, so it's safe regardless of what future callers pass through.
  - Requires a production redeploy, then running the new "Fix Untagged Form Submissions" tool from Super Admin, to fully take effect.
