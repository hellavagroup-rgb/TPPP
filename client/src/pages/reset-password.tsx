...
export default function ResetPassword() {
  ...
  useEffect(() => {
    if (!token) {
      setTokenInfo({ valid: false, error: "No reset token provided" });
      setIsLoading(false);
      return;
    }
    ...
    fetch(`/api/auth/reset-password/${token}`)
      ...
      .catch(() => {
        setTokenInfo({ valid: false, error: "Failed to validate reset link" });
        setIsLoading(false);
      });
  }, [token]);
      ...
      <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 p-4">
        <Card className="w-full max-w-md shadow-lg border-slate-200">
          ...
          <CardContent className="py-8 text-center">
            ...
            <p className="text-muted-foreground">{tokenInfo?.error || "This reset link is invalid or has expired."}</p>
          </CardContent>
  ...
  const brandName = tokenInfo.tenantName || GENERIC_APP_NAME;
  ...
  if (isComplete) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 p-4">
        <Card className="w-full max-w-md shadow-lg border-slate-200">
          <CardHeader className="space-y-4 items-center text-center pb-2">
            {tokenInfo.tenantLogoUrl ? (
              <img src={tokenInfo.tenantLogoUrl} alt={brandName} className="w-48 object-contain mb-2" />
            ) : (
      ...
      <Card className="w-full max-w-md shadow-lg border-slate-200">
        <CardHeader className="space-y-4 items-center text-center pb-2">
          ...
          <div className="space-y-1">
            ...
            <CardDescription>
              Choose a new password for {tokenInfo.email}.
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button className="w-full" type="submit" disabled={isSubmitting} data-testid="button-reset">
